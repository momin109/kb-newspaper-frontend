...
export type NewsCardVariant = 'default' | 'compact' | 'featured' | 'horizontal' | 'list'
    ...
    return (
      ...
      >
        <div className="relative h-16 w-24 shrink-0 overflow-hidden rounded-md">
          <Image src={imageUrl} alt={article.title} fill sizes="96px" className="object-cover" />
        </div>
  ...
  if (variant === 'horizontal') {
    return (
      <Link
        href={href}
        className={cn(
          'group flex w-64 shrink-0 flex-col overflow-hidden rounded-lg border border-border bg-card',
          className
        )}
      >
        <div className="relative h-36 w-full">
          <Image src={imageUrl} alt={article.title} fill sizes="256px" className="object-cover" />
        </div>
        <div className="flex flex-1 flex-col gap-1 p-3">
          <h4 className="line-clamp-2 text-sm font-semibold leading-snug group-hover:text-primary">
            {article.title}
          </h4>
          <span className="mt-auto text-xs text-muted-foreground">{timeLabel}</span>
        </div>
      </Link>
    )
  }
  ...
  if (variant === 'featured') {
    return (
      <Link
        href={href}
        className={cn(
          'group relative flex h-full min-h-[280px] flex-col justify-end overflow-hidden rounded-lg',
          className
        )}
      >
        <Image
        ...
        <div className="relative flex flex-col gap-2 p-4 sm:p-5">
          ...
          {showExcerpt && (
            <p className="line-clamp-2 text-sm text-white/85">{getExcerpt(article.content)}</p>
          )}
  ...
  if (variant === 'list') {
    return (
      <Link
        href={href}
        className={cn(
          'group flex gap-4 rounded-lg border border-border bg-card p-3 transition-shadow hover:shadow-md sm:p-4',
          className
        )}
      >
        <div className="relative h-24 w-32 shrink-0 overflow-hidden rounded-md sm:h-32 sm:w-48">
          <Image
            src={imageUrl}
            alt={article.title}
            fill
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1.5">
          {article.category && <CategoryBadge name={article.category.name} className="w-fit" />}
  ...
  return (
    ...
    >
      <div className="relative h-40 w-full sm:h-44">
        ...
        />
        {article.category && (
          <CategoryBadge name={article.category.name} className="absolute left-2 top-2" />
        )}
      </div>
