...
export function CategoryBadge({
  ...
  const colorClass =
    COLOR_BY_NAME[name] ?? "bg-primary text-primary-foreground";
  return (
    <span
      className={cn(
        "inline-flex w-fit items-center rounded-sm px-2 py-0.5 text-xs font-semibold tracking-wide shadow-sm",
        colorClass,
        className,
      )}
    >
