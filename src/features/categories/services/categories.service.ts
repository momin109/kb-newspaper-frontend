...
/**
 * Maps to the real backend: GET /api/category/all (routes/category.route.js).
...
export async function getCategories(): Promise<Category[]> {
  ...
  const res = await apiGet<ApiListEnvelope<Category>>('/category/all', { revalidate: 300 })
  return res.data
}
...
export async function getCategoryWithArticles(
  ...
  if (USE_MOCK) {
    ...
    const articles = MOCK_ARTICLES.filter((a) => a.category?._id === category._id).sort((a, b) =>
  ...
  try {
    const res = await apiGet<{ success: boolean; category: Category; articles: Article[] }>(
      `/category/all/${slug}`,
      { revalidate: 60 }
    )
